# Odoo 18  Module - Development Guide
...
- **Git**: Only commit and push when user requests only
- **port** 7096

## Quick Commands
```bash
# After any change (MANDATORY):
sudo docker restart odoo_skan
sudo docker exec -it odoo_skan odoo -u error_pages -d skan --stop-after-init
```
...
- **Globals**: Use `window.skanPortal = {...}` namespace to avoid conflicts
